...
class GazeFollow(Dataset):
    def __init__(self, data_dir, csv_path, transform, input_size=input_resolution, output_size=output_resolution,
                 test=False, imshow=False, no_aug=False):
        if test:
    ...
    def __getitem__(self, index):
        ...
        path_this = os.path.join(self.data_dir, path)
        ...
        head_coords = torch.IntTensor([int(x_min), int(y_min), int(x_max), int(y_max)])
        ...
        if self.imshow:
            ...
            plt.imshow(resize(gaze_heatmap, (self.input_size, self.input_size)), cmap='jet', alpha=0.3)
            plt.imshow(resize(1 - head_channel.squeeze(0), (self.input_size, self.input_size)), alpha=0.2)
            plt.savefig('viz_aug.png')

        if self.test:
            return img, face, head_channel, gaze_heatmap, cont_gaze, imsize, head_coords, path_this
        else:
...
class VideoAttTarget_video(Dataset):
    def __init__(self, data_dir, annotation_dir, transform, input_size=input_resolution, output_size=output_resolution,
                 test=False, imshow=False, seq_len_limit=400, save_ori_folder = ''):
        ...
        #self.all_sequence_paths = glob.glob(os.path.join(annotation_dir, '*.txt'))
        self.data_dir = data_dir
        self.transform = transform
        self.input_size = input_size
        self.output_size = output_size
        self.test = test
        self.depth_dir = "/nfs/bignet.cs.stonybrook.edu/add_disk0/qiaomu/datasets/Videoatt_Depth"
        self.imshow = imshow
        self.length = len(self.all_sequence_paths)
        self.seq_len_limit = seq_len_limit
        self.save_ori_folder = os.path.join(data_dir, "..", save_ori_folder)
        #if not os.path.exists(os.path.join(data_dir, "..", save_ori_folder)):
           #os.makedirs(os.path.join(data_dir, "..", save_ori_folder))
    ...
    def __getitem__(self, index):
        ...
        for i, row in df.iterrows():
            ...
            #for i in range(len(faces)):
                ...
                #img = np.clip(img, 0, 255)
                #plt.imshow(np.transpose(img, (1, 2, 0)))
                #plt.imshow(resize(heatmaps[i], (self.input_size, self.input_size)), cmap='jet', alpha=0.2)
                ##plt.imshow(imresize(1 - head_channels[i].squeeze(0), (self.input_size, self.input_size)), alpha=0.2)
                #plt.savefig(os.path.join(self.save_ori_folder, 'viz_%d_inout=%d.png' % (i, gaze_inouts[i])))
                #plt.close('all')
        ...
        if self.test:
            return images, faces, head_channels, depth_images, heatmaps, gazes, gaze_inouts, imsizes, head_coords, img_paths 
        else: # train
            return images, faces, head_channels, depth_images, heatmaps, gaze_inouts
