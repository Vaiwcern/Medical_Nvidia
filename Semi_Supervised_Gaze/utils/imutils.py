...
def get_interpolation(head_channels, out_size):
    if len(head_channels.size())>4:
        T, N, _, H, W = head_channels.size()
        head_channels = head_channels.view(-1, 1, H, W)
    head_channels_np = head_channels.squeeze(1).detach().cpu().numpy()
    feat_output= []
    with torch.no_grad():
        for i, head in enumerate(head_channels_np):
            head_new = resize(head, out_size, order=1)
            feat_output.append(torch.tensor(head_new).to(head_channels))
        feat_output = torch.stack(feat_output)
    return feat_output
...
def unnorm_batch(img, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    bs = img.shape[0]
    std = np.tile(np.array(std).reshape(3,1,1)[None:], (bs, 1, 1, 1))
    mean = np.tile(np.array(mean).reshape(3,1,1)[None:], (bs, 1, 1, 1))
    return img * std + mean
