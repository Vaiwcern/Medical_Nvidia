...
class GazeNet(nn.Module):
    def __init__(self, use_gradcam=False):
        ...
        new_kernel_channel = conv.data.mean(dim=1, keepdim=True).repeat(1, 3, 1, 1) if not self.use_gradcam else conv.data.mean(dim=1, keepdim=True).repeat(1, 4, 1, 1)
        new_kernel = torch.cat((conv.data, new_kernel_channel), 1)
        conv_channel = 6 if not self.use_gradcam else 7
        new_conv = nn.Conv2d(conv_channel, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
    ...
    def forward(self, x):
        image, face_image, gaze_field, eye_position, gradcam = x
        # face part forward
        ...
        if self.use_gradcam:
            image = torch.cat([image, gaze_field_map, gaze_field_map_2, gaze_field_map_3, gradcam], dim=1)
        else:
