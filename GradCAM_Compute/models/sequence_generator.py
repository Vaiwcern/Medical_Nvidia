...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        with torch.autograd.profiler.record_function("EnsembleModel: forward_encoder"):
            encoder_outs = model.forward_encoder(net_input, get_encoder_att=get_encoder_att)
            #import pdb
            #pdb.set_trace()
            if get_encoder_att:
                encoder_att_weights = [encoder_out['encoder_att_weights'] for encoder_out in encoder_outs]
...
class EnsembleModel(nn.Module):
    ...
    @torch.jit.export
    def forward_encoder(self, net_input: Dict[str, Tensor], get_encoder_att=False):
        if not self.has_encoder():
            return None

        return [model.encoder.forward_torchscript(net_input, get_encoder_att=get_encoder_att) for model in self.models]
