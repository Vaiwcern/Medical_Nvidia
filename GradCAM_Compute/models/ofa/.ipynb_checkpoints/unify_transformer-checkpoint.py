    ...
    def forward_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not get_encoder_att:
                ...
                x, att_layer = layer(x, encoder_padding_mask=encoder_padding_mask if has_pads else None, \
                        self_attn_bias=self_attn_bias, prompt_kv=prompt_kv, get_encoder_att=get_encoder_att)
                encoder_att_weights.append(att_layer)
                
            if return_all_hiddens:
                assert encoder_states is not None
                encoder_states.append(x)
        encoder_att_weights = torch.stack(encoder_att_weights)  # num_layer * seq_len * seq_len
        ...
        return {
            ...
            "encoder_att_weights": encoder_att_weights
        }
