...
def eval_vqa_gen(task, generator, models, sample, **kwargs):
    get_encoder_att = kwargs['get_encoder_att']
    if not get_encoder_att:
        hypos = task.inference_step(generator, models, sample, **kwargs)
    else:
        hypos, encoder_att_weights = task.inference_step(generator, models, sample, **kwargs)
    results = []
    for i, sample_id in enumerate(sample["id"].tolist()):
    ...
    if not get_encoder_att:
        ...
        return results, scores, encoder_att_weights

def eval_vqa_gen_returnall(task, generator, models, sample, **kwargs):
    get_encoder_att = kwargs['get_encoder_att']
    if not get_encoder_att:
        hypos = task.inference_step(generator, models, sample, **kwargs)
    else:
    ...
    for i, sample_id in enumerate(sample["id"].tolist()):
        result_beams = []
        attn_beams = []
        for beam_idx in range(len(hypos[i])):

            detok_hypo_str = decode_fn(hypos[i][beam_idx]["tokens"], task.tgt_dict, task.bpe, generator)
            attn = hypos[i][beam_idx]["attention"]
            score = hypos[i][beam_idx]['score']
            beam_pred = {"answer": detok_hypo_str.strip(), "score":score}
            result_beams.append(beam_pred)
            attn_beams.append(attn)
        results.append({"question_id": sample_id, "results": result_beams, "attention":attn_beams})
    scores = [results[0]['results'][i]['score'] for i in range(len(results[0]['results']))]
    if not get_encoder_att:
        return results, scores
    else:
        return results, scores, encoder_att_weights
...
def zero_shot_step(task, generator, models, sample, **kwargs):
    ...
    if task.cfg._name == 'vqa_gen':
        ...
        return eval_vqa_gen_returnall(task, generator, models, sample, **kwargs)
